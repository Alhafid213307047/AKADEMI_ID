...
import 'package:akademi_id/tesBakat/tb3_KemampuanAnalisisSintesis.dart';
import 'package:cloud_firestore/cloud_firestore.dart';
import 'package:flutter/material.dart';

class TBKemampuan_Verbal extends StatefulWidget {
  final int scoretb1;
  final String categoryTb1;

  const TBKemampuan_Verbal({
    super.key,
  ...
  @override
  State<TBKemampuan_Verbal> createState() => _TBKemampuan_VerbalState();
}

class _TBKemampuan_VerbalState extends State<TBKemampuan_Verbal> {
  ...
  void _startTimer() {
    _timer = Timer.periodic(Duration(seconds: 1), (timer) {
      if (_secondsLeft == 0) {
        ...
        int scoretb2 = _getScore(poin);
        int scoretb1 = widget.scoretb1;
        String categoryTb2 = _getResultCategory(scoretb2);
        String categoryTb1 = widget.categoryTb1;
        print('Total Poin: $poin');
        print('Nilai Score: $scoretb2');
        print('Kategori Hasil: $categoryTb2');
  ...
  void _showTimeUpDialog(
      int scoretb1, String categoryTb1, int scoretb2, String categoryTb2) {
    showDialog(
      context: context,
      barrierDismissible:
  ...
  String _getResultCategory(int score) {
    ...
    } else if (score >= 84 && score <= 95) {
      return 'Rendah';
    } else if (score >= 99 && score <= 114) {
      return 'Sedang';
    } else if (score >= 118 && score <= 130) {
      return 'Tinggi';
    } else if (score >= 134 && score <= 145) {
      ...
      child: Padding(
        ...
        child: Column(
          ...
          children: [
            Center(
              child: Text(
                'Instruksi Tes 02 - Kemampuan Verbal',
                style: TextStyle(
                    fontFamily: "Poppins",
                    fontWeight: FontWeight.bold,
                    fontSize: 16),
            ...
            SizedBox(height: 10),
            _buildOptionBox('a. meja'),
            _buildOptionBox('b. kursi'),
            _buildOptionBox('c. burung'),
            _buildOptionBox('d. lemari'),
            _buildOptionBox('e. tempat tidur'),
            SizedBox(height: 16),
            Text(
            ...
            SizedBox(height: 10),
            _buildOptionBox('a. duduk'),
            _buildOptionBox('b. berbaring'),
            _buildOptionBox('c. berdiri'),
            _buildOptionBox('d. berjalan'),
            _buildOptionBox('e. berjongkok'),
            SizedBox(height: 10),
            Text(
              'Pada a), b), c), dan e) prag berada dalam keadaan tidak bergerak, sedangkan d) orang dalam keadaan bergerak.',
              style: TextStyle(fontFamily: "Poppins"),
            ),
            SizedBox(height: 8),
            Text(
  ...
  Widget _buildQuestions() {
    return StreamBuilder<QuerySnapshot>(
      stream: FirebaseFirestore.instance
          .collection('TB Kemampuan Verbal')
          .snapshots(),
